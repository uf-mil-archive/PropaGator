...
GlobalPath::GlobalPath(const Point &start, const Limits &limits) : //todo: need or not ?
	q(start.q),
	qdot(start.qdot),
...
GlobalPath::PointWithAcceleration GlobalPath::getCurrentPoint() const { //todo: check needed or not
    return PointWithAcceleration(q, qdot, apply(GlobalPath::transformation_pair(q).second, qdotdot_b, 0));
}

void GlobalPath::update(double dt, const Waypoint &waypoint, double waypoint_t) { //todo: make your own update
...
/*double GlobalPath::c3filter(double q, double qdot, double qdotdot,
...
/*std::pair<Vector3d, Vector3d> GlobalPath::limit(const Vector3d &vmin, const Vector3d &vmax, const Vector3d &delta) {
